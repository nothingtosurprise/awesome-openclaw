...
- **Scoped autonomy:** The trusted agent may auto-merge only eligible README-only, non-structural, mechanically safe PRs that pass required checks.
...
This list differs from a general OSS list - it does not require OSI-approved licenses or star thresholds. The primary bar is: **clear OpenClaw relevance + a latest meaningful commit within 30 days**. See `CONTRIBUTING.md` for contributor-facing criteria and `.moltfounders/spec.txt` for automation decisions.
